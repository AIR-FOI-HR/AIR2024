    ...
    dbConnection.connection.query(sql, (err, data) => {
        if (err) {
            ...
            let list = data.map((activity) => {
                ...
                if (activity.locationName === "null" || activity.locationName === null) {
                    activity.locationName = "None";
                }
                return activity;
            })
...
router.post('/insert', (req, res, next) => {
    ...
    const handleTransaction = (connection, bool) => {
        ...
        connection.release();
        return bool ? res.status(201).json(true) : res.status(304).json(false);
    }
    ...
    dbConnection.connection.getConnection((err, connection) => {
        connection.beginTransaction((err) => {
            ...
            if (activity.LocationDetails) {
                locationSql = `INSERT INTO location VALUES ('default', '${activity.LocationDetails.locationName}', ${activity.LocationDetails.latitude}, ${activity.LocationDetails.longitude})`;
            } else {
                locationSql = `INSERT INTO location VALUES ('default', null, null, null)`;
            }
                                                    ...
                                                    connection.query(`INSERT INTO doing VALUES ('${mail}', ${activityId}, true)`, (err, result) => {
                                                        if (err) {
                                                            ...
                                                            if (activity.FinalDetails.supportedWeather.length !== 0) {
                                                                ...
                                                                connection.query(`INSERT INTO desiredforecast VALUES (${activityId}, 0)`, (err, result) => {
                                                                    if (err) {
                                                                        handleTransaction(connection, false);
                                                                    } else {
                                                                        handleTransaction(connection, true);
...
router.post('/update/:id', (req, res, next) => {
    ...
    if (activity.LocationDetails) {
        locationName = activity.LocationDetails.locationName;
        latitude = activity.LocationDetails.latitude;
        longitude = activity.LocationDetails.longitude;
    } else {
        locationName = latitude = longitude = null;
    }
    ...
    const handleTransaction = (connection, bool) => {
        bool ? connection.commit() : connection.rollback();
        connection.release();
        return bool ? res.json(true) : res.status(304).json(false);
    }
                        ...
                        if (err) {
                            ...
                            connection.query(
                                ...
                                SET a.startTime='${activity.TimeDetails.timeDetails.fromTime}', a.endTime='${activity.TimeDetails.timeDetails.untilTime}', a.title='${activity.FinalDetails.title}', a.description='${activity.FinalDetails.description}', a.categoryId=${categoryId}, a.type=${activity.FinalDetails.typeOfActivity},
                                    l.locationName='${locationName}', l.latitude=${latitude}, l.longitude=${longitude},
                                ...
                                WHERE a.activityId='${req.params.id}'`, (err, result) => {
                                    if (err) {
                                        ...
                                        if (activity.FinalDetails.supportedWeather.length !== 0) {
                                            ...
                                            connection.query(`INSERT INTO desiredforecast VALUES (${req.params.id}, 0)`, (err, result) => {
                                                if (err) {
                                                    handleTransaction(connection, false);
                                                } else {
                                                    handleTransaction(connection, true);
    ...
    dbConnection.connection.query(
        ...
        JOIN user u ON d.mail=u.mail WHERE u.sessionToken='${req.body.sessionToken}' AND a.activityId='${req.params.id}'`, (err, result) => {
            if (err) {
                ...
                return res.status(304).json(false);
            }
        }
    )
});
